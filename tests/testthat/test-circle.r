...
test_that("landmark sets agree", {
  expect_identical(
    landmarks_maxmin(s, num = nrow(s), engine = "original"),
...
test_that("landmark sets and cover sets agree", {
  expect_equal(
    landmarks_maxmin(s, num = nrow(s) / 2, engine = "C++", cover = TRUE),
    landmarks_maxmin(s, num = nrow(s) / 2, engine = "R", cover = TRUE)
  )
})
...
test_that("landmark sets agree", {
  expect_identical(
    landmarks_lastfirst(s, num = nrow(s), engine = "C++"),
    landmarks_lastfirst(s, num = nrow(s), engine = "R")
  )
})

test_that("landmark sets and cover sets agree", {
  expect_equal(
    landmarks_lastfirst(s, num = nrow(s) / 2, engine = "C++", cover = TRUE),
    landmarks_lastfirst(s, num = nrow(s) / 2, engine = "R", cover = TRUE)
  )
})
