...
test_that("landmarks are generated", {
  ...
  expect_silent(mm_lmk_cpp <- landmarks_maxmin_cpp(s, num_sets = nrow(s)))
  # maxmin landmarks in R
  expect_silent(mm_lmk_r <- landmarks_maxmin(s, n = nrow(s)))
})
...
test_that("landmarks are generated", {
  ...
  expect_silent(lf_lmk_cpp <- landmarks_lastfirst_cpp(s, num_sets = nrow(s)))
  # lastfirst landmarks in R
  expect_silent(lf_lmk_r <- landmarks_lastfirst_cory(s, number = nrow(s)))
})
