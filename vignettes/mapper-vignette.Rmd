...
Note that this vignette invokes the _currently installed_ version of the landmark package.
...
Compute the _pullback_ of the cover by identifying the points in $x \in X$ mapping into each cover set under $f$. 
...
for( i in seq_along(lmks$landmark) ){
  ...
  gg_cl_mm <- gg_cl_mm + geom_point(data=pts_cl, aes(x=x_1, y=x_2), color=colors_hex[i], alpha=0.1, size=3) +
    geom_text(data=pts_cl, aes(x=x_1, y=x_2, label=cluster), color=colors_hex[i])
}
...
for( i in seq_along(lmks$landmark) ){
  ...
  gg_cl_lf <- gg_cl_lf + geom_point(data=pts_cl, aes(x=x_1, y=x_2), color=colors_hex[i], alpha=0.1, size=3) +
    geom_text(data=pts_cl, aes(x=x_1, y=x_2, label=cluster), color=colors_hex[i])
}
