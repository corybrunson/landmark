  ...
  for (i in seq_along(datasets)) {
    ...
    mark <- mark(
      original = landmarks_maxmin(
        x, num_sets = num_sets[i], radius = radius, engine = "original"
      ),
      `C++` = landmarks_maxmin(
        x, num_sets = num_sets[i], radius = radius, engine = "C++"
      ),
      R = landmarks_maxmin(
        x, num_sets = num_sets[i], radius = radius, engine = "R"
      ),
      check = FALSE
    )
    mark <- mutate(
...
procedure_benchmark <- function(
  ...
  for (i in seq_along(datasets)) {
    x <- xs[[i]]
    mark <- mark(
      maxmin = landmarks_maxmin(x, num_sets = num_sets[i], engine = "R"),
      lastfirst = landmarks_lastfirst(x, num_sets = num_sets[i], engine = "R"),
      check = FALSE
    )
    mark <- mutate(
      mark,
...
# euclidean, twice binary log
procedure_benchmark(xs, num_sets = 2 * log(vapply(xs, nrow, 1L), 2))
```
