...
// //' @export
// // [[Rcpp::export]]
// IntegerVector landmarks_maxmin_cpp(const NumericMatrix& x, int num_sets = 0, float radius = -1, const int seed_index = 1) {
//     int num_pts = x.nrow();
//
//     // error handling
...
//         num_sets = num_pts;
//     }
//     if(num_sets == 1 && radius == -1){num_sets = std::min(num_pts,24);} // no parameters passed -> default behavior
...
// //' @export
// // [[Rcpp::export]]
// List landmarks_maxmin_cover_cpp(const NumericMatrix& x, int num_sets = 0, float radius = -1, const int seed_index = 1) {
//     int num_pts = x.nrow();
//
//     // error handling
//     if(radius < 0 && radius != -1){stop("Parameter 'radius' must be a positive number.");}
...
//
//     // store indices and values of X\L
//     map<int, vector<double>> X_all;
//     map<int, vector<double>> pts_left;
//     for(int i = 0; i < num_pts; i++){
//         NumericVector vec = x.row(i);
//         vector<double> point(vec.begin(),vec.end());
//         pts_left.emplace(i, point);
//         X_all.emplace(i, point);
//     }
//
//     // store indices and values of landmark set L
//     map<int, vector<double>> landmarks;
...
//             }
//             break;
//         } // TODO: this occurs one extra time because of c
//
//         // otherwise add new max to L and remove from X\L
//         pair<int, vector<double>> l_i = make_pair(maxmin.begin()->first, maxmin.begin()->second);
//         ordered_landmarks.push_back(l_i.first);
...
//     for(const auto& l : landmarks){
//         IntegerVector ball;
//         for(const auto& pt : X_all){
//             double d = dist_euc(l.second, pt.second);
//             if(d <= radius){
//                 ball.push_back(pt.first); // add pt to this ball
//             }
//         }
//         cover_sets.push_back(ball+1, to_string(l.first + 1));
//     }
//     return(cover_sets);
//     // // only return the indices of landmarks (not the values)
//     // IntegerVector ret = wrap(ordered_landmarks); // wrap into R data type
...
// //' @export
// // [[Rcpp::export]]
// List landmarks_lastfirst_cover_cpp(const NumericMatrix& x, int num_sets = 0, int cardinality = 0, const int seed_index = 1) {
//     int num_pts = x.nrow();
//
//     // additional parameter handling
//     if(num_sets > num_pts){
...
//     vector<int> ordered_landmarks; // keep track of order in which landmarks were added
//
//     map<int, vector<int>> nhds; // store k-nhds
//
//     // store indices and values of X\L
//     for(int i = 0; i < num_pts; i++){
//         NumericVector vec = x.row(i);
...
//         cover_sets.push_back(nhd+1, to_string(x+1));
//         //cover_sets[x] = wrap(nhds[x]);
//     }
//
//     //List retVals(2L);
//     //retVals[1] = ret;
